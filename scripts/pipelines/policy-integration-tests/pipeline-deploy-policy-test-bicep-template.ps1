...
AUTHOR: Tao Yang
DATE: 23/05/2024
NAME: pipeline-deploy-policy-test-bicep-template.ps1
VERSION: 1.0.0
...
Param (
  ...
  [Int64]$BuildNumber = $(Get-Random -Minimum 100000 -Maximum 999999),
...
$resourceGroupApiVersion = $globalTestConfig.resourceGroupApiVersion
...
if ($localTestConfig.ContainsKey('removeTestResourceGroup')) {
...
Write-Verbose "Test Subscription Id: $testSubId" -verbose

if ($localTestConfig.ContainsKey('tagsForResourceGroup')) {
...
if (Test-path $BicepFilePath -PathType Leaf) {
  ...
  if ($localTestConfig.additionalResourceGroups) {
    ...
    $rgReferenceNames = $localTestConfig.additionalResourceGroups.Keys
  ...
  Do {
    try {
      ...
      if ($provisioningState -ieq 'succeeded') {
        ...
        Write-Verbose "[$(getCurrentUTCString)]: The $templateScope scope deployment job provisioning state: '$provisioningState'." -verbose
      }
    } Catch {
  ...
  Write-Output "[$(getCurrentUTCString)]: Deployment Target: $deploymentTarget"
  Write-Output "[$(getCurrentUTCString)]: Template Scope: $templateScope"

  #create pipeline variables for deployment
  if ($runtimePlatform -ieq 'azure devops') {
    Write-Output "##vso[task.setVariable variable=bicepDeploymentName]$deploymentName"
    Write-Output "##vso[task.setVariable variable=bicepDeploymentName;isOutput=true]$deploymentName"

    Write-Output "##vso[task.setVariable variable=provisioningState]$provisioningState"
    Write-Output "##vso[task.setVariable variable=provisioningState;isOutput=true]$provisioningState"

    Write-Output "##vso[task.setVariable variable=bicepDeploymentId]$deploymentId"
    Write-Output "##vso[task.setVariable variable=bicepDeploymentId;isOutput=true]$deploymentId"

    Write-Output "##vso[task.setVariable variable=deploymentTarget]$deploymentTarget"
    Write-Output "##vso[task.setVariable variable=deploymentTarget;isOutput=true]$deploymentTarget"

    Write-Output "##vso[task.setVariable variable=bicepDeploymentScope]$templateScope"
    Write-Output "##vso[task.setVariable variable=bicepDeploymentScope;isOutput=true]$templateScope"
  } elseif ($runtimePlatform -ieq 'github actions') {
    Write-Output "deploymentName=$deploymentName" >> $env:GITHUB_OUTPUT
    Write-Output "provisioningState=$provisioningState" >> $env:GITHUB_OUTPUT
    Write-Output "bicepDeploymentId=$deploymentId" >> $env:GITHUB_OUTPUT
    Write-Output "deploymentTarget=$deploymentTarget" >> $env:GITHUB_OUTPUT
    Write-Output "bicepDeploymentScope=$templateScope" >> $env:GITHUB_OUTPUT
  }
  ...
  If ($deploymentResultFilePath -ne '') {
    ...
    if ($deploymentName -ne '') {
      ...
      $deploymentResult.Add('bicepDeploymentName', $deploymentName)
    }
...
#Always create deploymentOutputs and deploymentId pipeline variable even if there are no outputs So it can be used in the next steps
if ($runtimePlatform -ieq 'azure devops') {
  Write-Output "##vso[task.setVariable variable=bicepDeploymentOutputs]$deploymentOutputs"
  Write-Output "##vso[task.setVariable variable=bicepDeploymentOutputs;isOutput=true]$deploymentOutputs"
} elseif ($runtimePlatform -ieq 'github actions') {
  Write-Output "bicepDeploymentOutputs=$deploymentOutputs" >> $env:GITHUB_OUTPUT
}
