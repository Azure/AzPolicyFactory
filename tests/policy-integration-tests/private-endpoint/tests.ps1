...
$pePolicyAssignmentId = $script:LocalConfig_policyAssignmentIds | Where-Object { $_ -imatch "$script:LocalConfig_peAssignmentName`$" }
$amplsViolatingPolicies = @(
  @{
    policyAssignmentId          = $pePolicyAssignmentId
    policyDefinitionReferenceId = 'PE-002'
  }
)
